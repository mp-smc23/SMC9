...
public:
	...
	int samplesForOutput(int outputSamples) const {
		float inputSamples = outputSamples*invTimeFactor - surplusInputSamples;
...
public:
	...
	void configure(int channels, int blockSamples, int intervalSamples, float zeroPadding=1, int maxExtraInput=0) {
		OverlapAddStretch::configure(channels, blockSamples, intervalSamples, maxExtraInput);
...
public:
	...
	void setFreqFactor(float factor) {
		freqFactor = factor;
	}
protected:
...
private:
	...
	Sample getEnergy(Complex *spectrum, float index) {
	...
	Complex getBin(Complex *spectrum, float index) {
		int indexFloor = int(std::floor(index));
		Sample fractional = index - indexFloor;
...
public:
	HybridPhaseStretch(bool multipleTimeObservations=true, float pitchWeight=1, float timeWeight=2, float channelWeight=1, float maxWeight=1) : multipleTimeObservations(multipleTimeObservations), pitchWeight(pitchWeight), timeWeight(timeWeight), channelWeight(channelWeight), maxWeight(maxWeight) {}
...
private:
	...
	float freqFactor = 1;
	float pitchWeight, timeWeight, channelWeight, maxWeight;
